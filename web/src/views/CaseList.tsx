...
const CaseRow = ({
  ...
  <li>
    ...
    <span>
      {caseView.status} · revision {caseView.revision}
    </span>
    <CopyValue label="case reference" value={caseView.caseReference} />
  </li>
);
...
const LookupForm = ({
  ...
  >
    <TextField value={lookup} onChange={onChange}>
      <Label>Exact case reference</Label>
      <Input id="case-lookup" autoComplete="off" />
    </TextField>
    <Button type="submit">Find case</Button>
    <Button onPress={onReload} isDisabled={loading}>
      {loading ? "Loading…" : "Reload cases"}
    </Button>
  </Form>
);
  ...
  return (
    <section aria-labelledby="case-list-title">
      <div className="section-heading">
        ...
        <Button onPress={onOpen}>Open new case</Button>
      </div>
      <LookupForm
        lookup={lookup}
        loading={loading}
